...
function carValidMove(_this) {
  ...
  while (true) {
    ...
    if (rightCheck.dataset.country) {
      if (rightCheck.dataset.country != turn)
  ...
  if ([0, 7].includes(x) && y == 3) {
    ...
    [1, -1].forEach((r) => {
      [1, -1].forEach((c) => {
        ...
        if (
          (rightCheck && !rightCheck.dataset.country) ||
          (rightCheck && rightCheck.dataset.country != turn)
        )
          giveValidWithNotChecked(_this, rightCheck);
      });
    });
...
function horseValidMove(_this) {
  ...
  if (data[x + 1]?.[y] && !data[x + 1]?.[y].dataset.country) {
    if (
      (data[x + 2]?.[y + 1] && !data[x + 2]?.[y + 1].dataset.country) ||
      (data[x + 2]?.[y + 1] && data[x + 2]?.[y + 1].dataset.country != turn)
    )
      giveValidWithNotChecked(_this, data[x + 2]?.[y + 1]);
    if (
      (data[x + 2]?.[y - 1] && !data[x + 2]?.[y - 1].dataset.country) ||
      (data[x + 2]?.[y - 1] && data[x + 2]?.[y - 1].dataset.country != turn)
    )
  ...
  if (data[x]?.[y + 1] && !data[x]?.[y + 1].dataset.country) {
    if (
      (data[x + 1]?.[y + 2] && !data[x + 1]?.[y + 2].dataset.country) ||
      (data[x + 1]?.[y + 2] && data[x + 1]?.[y + 2].dataset.country != turn)
    )
      giveValidWithNotChecked(_this, data[x + 1]?.[y + 2]);
    if (
      (data[x - 1]?.[y + 2] && !data[x - 1]?.[y + 2].dataset.country) ||
      (data[x - 1]?.[y + 2] && data[x - 1]?.[y + 2].dataset.country != turn)
    )
...
function elephantValidMove(_this) {
  ...
  if (data[x + 1]?.[y] && !data[x + 1]?.[y].dataset.country) {
    if (data[x + 2]?.[y + 1] && !data[x + 2]?.[y + 1].dataset.country) {
      if (
        (data[x + 3]?.[y + 2] && !data[x + 3]?.[y + 2].dataset.country) ||
        (data[x + 3]?.[y + 2] && data[x + 3]?.[y + 2].dataset.country != turn)
      )
        giveValidWithNotChecked(_this, data[x + 3]?.[y + 2]);
    }
    if (data[x + 2]?.[y - 1] && !data[x + 2]?.[y - 1].dataset.country) {
      if (
        (data[x + 3]?.[y - 2] && !data[x + 3]?.[y - 2].dataset.country) ||
  ...
  if (data[x]?.[y + 1] && !data[x]?.[y + 1].dataset.country) {
    if (data[x + 1]?.[y + 2] && !data[x + 1]?.[y + 2].dataset.country) {
      if (
        (data[x + 2]?.[y + 3] && !data[x + 2]?.[y + 3].dataset.country) ||
        (data[x + 2]?.[y + 3] && data[x + 2]?.[y + 3].dataset.country != turn)
      )
        giveValidWithNotChecked(_this, data[x + 2]?.[y + 3]);
    }
    if (data[x - 1]?.[y + 2] && !data[x - 1]?.[y + 2].dataset.country) {
      if (
        (data[x - 2]?.[y + 3] && !data[x - 2]?.[y + 3].dataset.country) ||
...
function cannonValidMove(_this) {
  ...
  while (true) {
    ...
    if (
      !pivot &&
      rightCheck.dataset.country &&
      !rightCheck.classList.contains("cannon")
    ) {
      pivot = true;
      i++;
      continue;
    ...
    if (rightCheck.dataset.country) {
      if (
        rightCheck.dataset.country != turn &&
        !rightCheck.classList.contains("cannon")
      )
        giveValidWithNotChecked(_this, rightCheck);
      break;
  ...
  if (
    [0, 7].includes(x) &&
    y == 3 &&
    data[x + 1][y + 1].dataset.country &&
    !data[x + 1][y + 1].classList.contains("cannon")
  ) {
...
function kingValidMove(_this) {
  ...
  const y = _this.cellIndex;
  if (
    ([0, 2, 7, 9].includes(x) && [3, 5].includes(y)) ||
    ([1, 8].includes(x) && y == 4)
  ) {
    [1, 0, -1].forEach((i) => {
      [1, 0, -1].forEach((j) => {
        ...
        if (
          rightCheck &&
          rightCheck.classList.contains("house") &&
          !(i == 0 && j == 0) &&
          (!rightCheck.dataset.country ||
            (rightCheck && rightCheck.dataset.country != turn))
        ) {
...
function whichMoveToSelect() {
  ...
  if (this.classList.contains("car")) {
    carValidMove(_this);
    return;
  }
  if (this.classList.contains("soldier")) {
    soldierValidMove(_this);
    return;
  }
  if (this.classList.contains("horse")) {
    horseValidMove(_this);
    return;
  }
  if (this.classList.contains("elephant")) {
    elephantValidMove(_this);
    return;
  }
  if (this.classList.contains("cannon")) {
    cannonValidMove(_this);
    return;
  }
  ...
  if (this.classList.contains("scholar")) {
    scholarValidMove(_this);
    return;
  } //그게 아니라면 움직일 수 있는 칸 표시해주기.
}
...
function isGameEnd() {
  return (
    data.flat().every((e) => {
      if (e.dataset.country == turn) {
        if (e.classList.contains("car")) {
          carValidMove(e);
          let isThereValid = data
            .flat()
            .some((el) => el.classList.contains("valid"));
          carValidMove(e);
          return !isThereValid;
        }
    ...
    diePiecesNameForCho.includes("king") ||
    diePiecesNameForHan.includes("king")
  ); //한수쉼 해서 왕이 먹힐수도 있음
}
