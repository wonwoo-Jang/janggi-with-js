...
function cannonValidMove(_this) {
  ...
  while (true) {
    ...
    if (rightCheck.dataset.country) {
      if (rightCheck.dataset.country != turn && !rightCheck.classList.contains('cannon'))
        giveValidWithNotChecked(_this, rightCheck);
      break;
    } //두번째로 만나는 장애물까지 이동가능
    if (pivot) {
  ...
  if (
    ...
    if (
      ...
      (rightCheck && rightCheck.dataset.country != turn && !rightCheck.classList.contains('cannon'))
    )
      giveValidWithNotChecked(_this, rightCheck);
  } else if (
    [0, 7].includes(x) &&
